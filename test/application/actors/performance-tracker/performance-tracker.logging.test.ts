...
const createMockPerformanceTrackerActorDefinition = () => ({
  initialState: {
    trades: [] as TradeEntry[],
    openTrades: {} as Record<string, TradeEntry>,
  ...
  behavior: async (
    ...
    switch (msg.type) {
      case "TRADE_OPENED":
        if (msg.trade) {
          newState.trades = [...state.trades, msg.trade];
          newState.openTrades = {
            ...state.openTrades,
            [msg.trade.id]: msg.trade,
          };
        }
        break;
      case "TRADE_CLOSED":
        if (msg.trade) {
          newState.trades = state.trades.map((t: TradeEntry) =>
            t.id === msg.trade!.id ? msg.trade! : t,
          );
          const { [msg.trade.id]: removed, ...remainingTrades } =
            state.openTrades;
          newState.openTrades = remainingTrades;

          // Update strategy performance
          if (!newState.strategyPerformance[msg.trade.strategyId]) {
            newState.strategyPerformance[msg.trade.strategyId] = {
              trades: 0,
              netProfit: 0,
            };
          }
          newState.strategyPerformance[msg.trade.strategyId].trades += 1;
          newState.strategyPerformance[msg.trade.strategyId].netProfit +=
            msg.trade.pnl || 0;
        }
        break;
      case "UPDATE_PRICE":
        if (msg.symbol && msg.price !== undefined && msg.timestamp) {
          if (!newState.priceHistory[msg.symbol]) {
            newState.priceHistory[msg.symbol] = [];
          }
          newState.priceHistory[msg.symbol].push({
            price: msg.price,
      ...
      case "UPDATE_CONFIG":
        if (msg.config) {
          newState.config = { ...state.config, ...msg.config };
        }
        break;
      case "LOG_POSITIONS_SUMMARY":
        // For logging, we don't change state
        break;
    }
...
describe("PerformanceTracker Logging", () => {
  ...
  test("should log trade closing with PNL information", async () => {
    const openTrade = createSampleTrade(
      "test-002",
      "elliott-strategy",
      "ETH-USD",
      3000,
    ...
    const openResult = await actorDefinition.behavior(
      state,
      { payload: openMessage },
      mockContext,
    );
    ...
    const closeResult = await actorDefinition.behavior(
      state,
    ...
    expect(
      closeResult.state.strategyPerformance["elliott-strategy"],
    ).toBeDefined();
  });
  ...
  test("should update price and log unrealized PNL", async () => {
    ...
    const priceResult = await actorDefinition.behavior(
      state,
  ...
  test("should generate positions summary", async () => {
    const trade1 = createSampleTrade(
      "test-004",
      "rsi-strategy",
      "BTC-USD",
      45000,
    );
    const trade2 = createSampleTrade(
      "test-005",
      "elliott-strategy",
      "ETH-USD",
      3000,
    ...
    const result1 = await actorDefinition.behavior(
      state,
      { payload: openMessage1 },
      mockContext,
    );
    const result2 = await actorDefinition.behavior(
      result1.state,
      { payload: openMessage2 },
      mockContext,
    );
    ...
    const summaryResult = await actorDefinition.behavior(
      result2.state,
  ...
  test("should track statistics by strategy", async () => {
    const rsiTrade1 = createSampleTrade(
      "rsi-001",
      "rsi-strategy",
      "BTC-USD",
      45000,
    );
    const rsiTrade2 = createSampleTrade(
      "rsi-002",
      "rsi-strategy",
      "ETH-USD",
      3000,
    );
    const elliottTrade = createSampleTrade(
      "elliott-001",
      "elliott-strategy",
      "BTC-USD",
      44000,
    ...
    const finalResult = await actorDefinition.behavior(
      state,
      { payload: closeMessage },
      mockContext,
    );
    ...
    expect(finalResult.state.strategyPerformance["rsi-strategy"].trades).toBe(
      1,
    ); // Only closed trades count
    expect(
      finalResult.state.strategyPerformance["rsi-strategy"].netProfit,
    ).toBe(1000);
