...
const strategyConfigSchema = z.object({
  type: z.enum(['rsi-divergence', 'volume-analysis', 'elliott-wave', 'harmonic-pattern']),
    ...
    strategies: symbols.map(symbol => [
      {
        ...
        parameters: {
          period: Number(process.env.RSI_PERIOD || STRATEGY_CONSTANTS.RSI.DEFAULT_PERIOD),
          ...
          divergenceLookback: Number(process.env.RSI_DIVERGENCE_LOOKBACK || STRATEGY_CONSTANTS.RSI.DIVERGENCE_LOOKBACK),
          symbol: symbol.trim(),
      ...
      },
      {
        ...
        parameters: {
          maPeriod: Number(process.env.VOLUME_MA_PERIOD || STRATEGY_CONSTANTS.VOLUME.MA_PERIOD),
          spikeThreshold: Number(process.env.VOLUME_SPIKE_THRESHOLD || STRATEGY_CONSTANTS.VOLUME.SPIKE_THRESHOLD),
      ...
      },
      {
        ...
        parameters: {
          minWaveLength: Number(process.env.ELLIOTT_MIN_WAVE_LENGTH || STRATEGY_CONSTANTS.ELLIOTT.MIN_WAVE_LENGTH),
