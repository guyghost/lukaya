...
export class BacktestIntegrationService {
  ...
  async validateAndAddStrategy(
    ...
    try {
      ...
      const validationResults = {
        profitPercentage: result.profitPercentage >= criteria.minProfitPercentage,
        winRate: result.metrics.winRate >= criteria.minWinRate,
        drawdown: result.metrics.maxDrawdownPercentage <= criteria.maxDrawdownPercentage,
      ...
      if (passed) {
        ...
        if (!validationResults.profitPercentage) {
          failedCriteria.push(`Profit (${result.profitPercentage.toFixed(2)}%) < ${criteria.minProfitPercentage}%`);
        }
        
        if (!validationResults.winRate) {
          failedCriteria.push(`Win rate (${(result.metrics.winRate * 100).toFixed(2)}%) < ${criteria.minWinRate * 100}%`);
        }
        
        if (!validationResults.drawdown) {
          failedCriteria.push(`Drawdown (${result.metrics.maxDrawdownPercentage.toFixed(2)}%) > ${criteria.maxDrawdownPercentage}%`);
        }
        
        if (!validationResults.sharpeRatio) {
          failedCriteria.push(`Sharpe ratio (${result.metrics.sharpeRatio.toFixed(2)}) < ${criteria.minSharpeRatio}`);
        }
   ...
   */
  async validateActiveStrategies(
    config: BacktestConfig
  ): Promise<{
    ...
    // @ts-ignore
    const activeStrategies = this.tradingBotService.getActiveStrategies?.() || [];
    ...
    for (const strategy of activeStrategies) {
      ...
      try {
        ...
        if (result.profitPercentage <= 0) {
        ...
        } else if (result.profitPercentage < 3 || result.metrics.winRate < 0.45) {
          ...
          recommendations[strategy.getId()] = {
            ...
            optimizationSuggestions: {
              ...
              stopLoss: result.metrics.maxDrawdownPercentage > 10 ? 
                Math.min(Math.ceil(result.metrics.maxDrawdownPercentage / 2), 5) : 
                undefined,
              takeProfit: result.metrics.averageWin > 0 ?
                Math.ceil(result.metrics.averageWin / result.initialBalance * 100) :
                undefined
            }
          };
        } else {
