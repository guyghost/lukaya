...
import { IntervalId } from "../../shared/types";
...
export const createTakeProfitIntegrator = (
  ...
  const takeProfitManager: TakeProfitPort = {
    updateConfig: async (config) => {
      actorSystem.send(takeProfitManagerActor, {
        type: "UPDATE_CONFIG",
        config,
      });
    },
    ...
    analyzePosition: async (symbol, currentPrice) => {
      ...
      return {
        ...
        symbol,
        reason: "Analyse non implémentée",
      };
    },
  ...
  let priceCheckIntervalId: IntervalId | null = null;
  let positionCheckIntervalId: IntervalId | null = null;
  ...
  const updatePositionsFromRiskManager = async () => {
    try {
      ...
      for (const symbol in knownPositions) {
        const isStillOpen = accountRisk.positions.some(
          (p) => p.symbol === symbol,
        );
        if (!isStillOpen) {
  ...
  const checkPriceUpdates = async () => {
    for (const symbol of watchedSymbols) {
      try {
        ...
        logger.error(
          `Erreur lors de la vérification du prix pour ${symbol}:`,
          error as Error,
        );
      }
    }
  ...
  const start = () => {
    ...
    if (!priceCheckIntervalId) {
      priceCheckIntervalId = setInterval(
        checkPriceUpdates,
        mergedConfig.priceCheckInterval,
      );
    }
