...
import { ParameterRanges, ParameterCombination } from "../../shared/types";
...
export class BacktestController {
  ...
  async runBacktest(
    ...
    this.logger.info(
      `Démarrage du backtest pour la stratégie ${strategy.getName()}`,
    );
  ...
  async compareStrategies(
    ...
    this.logger.info(
      `Comparaison de ${strategies.length} stratégies sur ${config.symbol}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Erreur lors de la comparaison des stratégies:`,
        error as Error,
      );
      throw error;
    }
  ...
  async optimizeStrategy(
    ...
    bestParameters: ParameterCombination;
    ...
    this.logger.info(
      `Optimisation des paramètres pour la stratégie ${strategy.getName()}`,
    );
    ...
    try {
      ...
      this.logger.info(
        `${parameterCombinations.length} combinaisons de paramètres à tester`,
      );
      ...
      for (const parameters of parameterCombinations) {
        ...
        const strategyWithParams = this.updateStrategyParameters(
          strategy,
          parameters,
        );
      ...
      this.logger.error(
        `Erreur lors de l'optimisation de la stratégie ${strategy.getName()}:`,
        error as Error,
      );
      throw error;
    }
  ...
  private updateStrategyParameters(
    ...
    this.logger.warn(
      `La stratégie ${strategy.getName()} ne supporte pas la mise à jour des paramètres`,
    );
    return strategy;
  }
}
