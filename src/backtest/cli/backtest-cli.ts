...
export class BacktestCli {
    ...
    try {
      switch (command) {
        ...
        case 'run': {
          ...
          if (!result.success) {
            this.logger.error(result.message || 'Erreur lors de l\'exécution du backtest');
        ...
        case 'compare': {
          ...
          if (!result.success) {
            this.logger.error(result.message || 'Erreur lors de la comparaison des stratégies');
            console.error(`Erreur: ${result.message}`);
            process.exit(1);
          }
          break;
        ...
        case 'optimize': {
          ...
          if (!result.success) {
            this.logger.error(result.message || 'Erreur lors de l\'optimisation de la stratégie');
            console.error(`Erreur: ${result.message}`);
            process.exit(1);
          }
          break;
  ...
  private async runBacktest(args: string[]): Promise<Result<void>> {
    ...
    const backtestResult = await this.backtestAdapter.runBacktest(options.strategy, config);
    ...
    if (format === 'json') {
      ...
      const report = this.backtestAdapter.generateBacktestReport(backtestResult);
      console.log(report);
    }
    ...
    if (options.output) {
      if (format === 'json') {
        this.backtestAdapter.exportBacktestResults(backtestResult, options.output);
      } else {
        const report = this.backtestAdapter.generateBacktestReport(backtestResult);
