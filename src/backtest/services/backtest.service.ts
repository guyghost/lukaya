...
export class BacktestService {
  ...
  async runBacktest(
    ...
    this.logger.info(
      `Démarrage du backtest pour la stratégie ${strategy.getName()} sur ${config.symbol}`,
    );
    ...
    try {
      ...
      this.logger.info(
        `${historicalData.length} points de données historiques chargés`,
      );

      // Initialiser la stratégie avec les premières données si nécessaire
      if (strategy.initializeWithHistory) {
        const initializationData = historicalData.slice(
          0,
          Math.min(100, historicalData.length),
        );
        await strategy.initializeWithHistory(initializationData);
        this.logger.info(
          `Stratégie initialisée avec ${initializationData.length} points de données`,
        );
      }
      ...
      for (const data of historicalData) {
        ...
        if (position && position.status === "open") {
          const isStopLossTriggered = this.checkStopLoss(
            position,
            data,
            config,
          );
          const isTakeProfitTriggered = this.checkTakeProfit(
            position,
            data,
            config,
          );
        ...
        if (signal) {
          this.logger.debug(
            `Signal généré à ${new Date(data.timestamp).toISOString()}: ${signal.type} ${signal.direction}`,
          );
      ...
      this.logger.info(
        `Backtest terminé pour ${strategy.getName()}. Profit: ${result.profit.toFixed(2)} (${result.profitPercentage.toFixed(2)}%)`,
      );

      return result;
    } catch (error) {
      this.logger.error(
        `Erreur lors du backtest de la stratégie ${strategy.getName()}:`,
        error as Error,
      );
      throw error;
    }
  ...
  private async loadHistoricalData(
    ...
    this.logger.info(
      `Chargement des données historiques pour ${config.symbol} du ${config.startDate.toISOString()} au ${config.endDate.toISOString()}`,
    );
    ...
    try {
      ...
      if (cachedData && cachedData.length > 0) {
        this.logger.info(
          `Données chargées depuis le cache: ${cachedData.length} points`,
        );
      ...
      if (apiData && apiData.length > 0) {
        ...
        this.logger.info(
          `Données chargées depuis l'API: ${apiData.length} points`,
        );
      ...
      this.logger.warn(
        `Aucune donnée réelle disponible pour ${config.symbol}, génération de données synthétiques`,
      );
      ...
      throw new Error(
        `Impossible de charger les données historiques: ${(error as Error).message}`,
      );
    }
  }
  ...
  private async saveToCache(
    ...
    try {
      ...
      this.logger.warn(
        `Erreur lors de la sauvegarde des données dans le cache:`,
        error as Error,
      );
    }
  }
  ...
  private async loadFromApi(
    ...
    try {
      ...
      const marketData = this.transformApiResponseToMarketData(
        response.data,
        config.symbol,
      );
      return marketData;
   ...
   */
  private transformApiResponseToMarketData(
    apiData: unknown[],
    symbol: string,
  ): MarketData[] {
  ...
  private generateSyntheticData(config: BacktestConfig): MarketData[] {
    this.logger.info(
      `Génération de données synthétiques pour ${config.symbol}`,
    );
    ...
    const timeInterval =
      intervalMap[config.dataResolution] || intervalMap["1h"];
   ...
   */
  private preprocessHistoricalData(
    data: MarketData[],
    config: BacktestConfig,
  ): MarketData[] {
    ...
    this.logger.info(
      `Prétraitement terminé: ${weekendFilteredData.length} points de données utilisables`,
    );
  ...
  private checkStopLoss(
    ...
    if (position.direction === "long") {
      const lossPercentage =
        (position.entryPrice - data.price) / position.entryPrice;
      return lossPercentage >= stopLossThreshold;
    } else {
      // short
      const lossPercentage =
        (data.price - position.entryPrice) / position.entryPrice;
      return lossPercentage >= stopLossThreshold;
    }
  }
  ...
  private closePosition(
    ...
    const exitFees = this.calculateFees(
      exitPrice,
      position.quantity,
      config.tradingFees,
    );
    ...
    const profitPercentage =
      (profit / (position.entryPrice * position.quantity)) * 100;
  ...
  private calculateMetrics(
    ...
    const holdingTimes = closedTrades.map((t) =>
      t.exitDate && t.entryDate
        ? t.exitDate.getTime() - t.entryDate.getTime()
        : 0,
    );
  ...
  private calculateSharpeRatio(trades: BacktestTrade[]): number {
    ...
    const variance =
      squaredDeviations.reduce((a, b) => a + b, 0) / (returns.length - 1);
  ...
  private calculateSortinoRatio(trades: BacktestTrade[]): number {
    ...
    const squaredNegativeDeviations = negativeReturns.map((r) =>
      Math.pow(r, 2),
    );
    const downVariance =
      squaredNegativeDeviations.reduce((a, b) => a + b, 0) /
