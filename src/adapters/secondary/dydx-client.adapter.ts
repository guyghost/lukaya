...
const DEFAULT_QUANTUM_CONFIG: Record<
  string,
  { atomicResolution: number; stepBaseQuantums: number }
> = {
...
export const createDydxClient = (
  config: DydxClientConfig,
): {
  ...
  const initializeClient = async (): Promise<CompositeClient> => {
    if (!context.compositeClient) {
      ...
      try {
        await executeWithRateLimit(
          async () => {
            const marketsResponse =
              await context.compositeClient!.indexerClient.markets.getPerpetualMarkets();
            if (marketsResponse.markets) {
              ...
              logger.debug(
                `Chargé ${context.markets.size} marchés depuis dYdX`,
              );
            }
          },
          context.rateLimiter,
        ...
        logger.error(
          "Échec du chargement des configurations de marché :",
          error as Error,
        );
      }
    }
  ...
  const getWallet = async (): Promise<LocalWallet> => {
    if (!context.localWallet && config.mnemonic) {
      logger.debug(
        "Initialisation du portefeuille local à partir de la phrase mnémonique",
      );
      context.localWallet = await LocalWallet.fromMnemonic(
        config.mnemonic,
        BECH32_PREFIX,
      );
    }
    ...
    if (!context.localWallet) {
      throw new Error(
        "Le portefeuille n'est pas initialisé. Veuillez fournir une phrase mnémonique.",
      );
    }
  ...
  const getMarketConfig = (symbol: string): ExternalMarketData => {
    ...
    try {
      ...
      try {
        ...
        atomicResolution = market.atomicResolution
          ? parseInt(market.atomicResolution)
          : -6;
        stepBaseQuantums = market.stepBaseQuantums
          ? parseInt(market.stepBaseQuantums)
          : 1000000;
      } catch (marketError) {
        // Use default configuration if market config unavailable
        logger.warn(
          `Market config unavailable for ${symbol}, using defaults:`,
          marketError as Error,
        );
        ...
        if (defaultConfig) {
          ...
          logger.info(`Using default quantum config for ${symbol}:`, {
            atomicResolution,
            stepBaseQuantums,
          });
        } else {
          ...
          logger.warn(
            `No default config for ${symbol}, using conservative fallback`,
          );
        }
      }
      ...
      if (size < minQuantumSize) {
        ...
        logger.warn(
          `⚠️ Order size ${size} below minimum ${minQuantumSize} for ${symbol}`,
          {
            adjustedSize,
      ...
      if (remainder !== 0) {
        ...
        const adjustedQuantums =
          Math.round(quantums / stepBaseQuantums) * stepBaseQuantums;
        const adjustedSize = adjustedQuantums * Math.pow(10, atomicResolution);

        logger.info(
          `🔧 Aligning order size to quantum increments for ${symbol}:`,
          {
            originalSize: size,
      ...
      logger.error(
        `Failed to validate quantums for ${symbol}:`,
        error as Error,
      );
      ...
      if (defaultConfig) {
        conservativeMin =
          defaultConfig.stepBaseQuantums *
          Math.pow(10, defaultConfig.atomicResolution);
      }
  ...
  const mapOrderType = (type: OrderType): DydxOrderType => {
    ...
    switch (type) {
      ...
      case OrderType.STOP:
        ...
        logger.info(
          "✅ TRAÇAGE MAPPING - Mappé vers DydxOrderType.STOP_LIMIT :",
          {
            input: type,
            output: result,
          },
  ...
  const mapDydxOrderToOrder = (dydxOrder: ExternalOrderData): Order => {
    ...
    return {
      ...
      side:
        dydxOrder.side === DydxOrderSide.BUY ? OrderSide.BUY : OrderSide.SELL,
      ...
      avgFillPrice: dydxOrder.avgPrice
        ? parseFloat(dydxOrder.avgPrice)
        : undefined,
      ...
      const marketData = await executeWithRateLimit(
        async () => {
          ...
          const marketResponse =
            await indexerClient.markets.getPerpetualMarkets();
          ...
          const candles = await indexerClient.markets.getPerpetualMarketCandles(
            market.ticker,
            "1MIN",
          );
          ...
          // Get orderbook
          const orderbook =
            await indexerClient.markets.getPerpetualMarketOrderbook(symbol);
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (marketData) {
          ...
          logger.info(
            `[PERF] ${symbol} tick #${metrics.tickCount} | Δt=${tickDuration}ms | avgΔt=${(metrics.totalTickDuration / metrics.tickCount).toFixed(1)}ms | errors=${metrics.errorCount}`,
          );
        ...
        errorCount += 1;
        logger.error(
          `Error during market data polling for ${symbol}:`,
          error as Error,
        );
        // Exponential backoff: min 10s, max 5min
        const delay = Math.min(300000, 10000 * Math.pow(2, errorCount - 1));
        logger.warn(
          `[BACKOFF] ${symbol} polling delayed by ${delay / 1000}s after ${errorCount} error(s)`,
        );
  ...
  const getDefaultSubaccount = async (): Promise<Record<string, unknown>> => {
    const client = await getClient();
    const wallet = await getWallet();
    ...
    const subaccountResponse =
      await client.indexerClient.account.getSubaccounts(wallet.address);
  ...
  const marketDataPort: MarketDataPort = {
    subscribeToMarketData: async (symbol: string): Promise<void> => {
      ...
      try {
        ...
        logger.error(
          `Échec de l'abonnement aux données de marché pour ${symbol} :`,
          error as Error,
        );
        throw error;
      }
    ...
    getHistoricalMarketData: async (
      ...
      try {
        ...
        logger.debug(
          `Fetching historical data for ${symbol} with limit ${limit}`,
        );

        // Fetch historical candles (e.g., 1MIN interval)
        const candlesResponse =
          await indexerClient.markets.getPerpetualMarketCandles(symbol, "1MIN");
        if (
          !candlesResponse?.candles ||
          !Array.isArray(candlesResponse.candles)
        ) {
          throw new Error(`No historical candle data found for ${symbol}`);
        }

        logger.debug(
          `Received ${candlesResponse.candles.length} candles for ${symbol}`,
        );
        ...
        const mappedData = candlesResponse.candles
          ...
          .map((candle: ExternalMarketData, index: number) => {
            ...
            if (candle.usdVolume !== undefined && candle.usdVolume !== null) {
              ...
              candle.baseTokenVolume !== undefined &&
              candle.baseTokenVolume !== null
            ) {
              // Fallback to baseTokenVolume if usdVolume not available
              const parsedVolume = parseFloat(
                candle.baseTokenVolume.toString(),
              );
              volume = isNaN(parsedVolume) ? 0 : Math.max(0, parsedVolume);
          ...
          .filter(
            (data: MarketData | null): data is MarketData => data !== null,
          ); // Remove any null entries

        logger.debug(
          `Mapped ${mappedData.length} valid data points for ${symbol}`,
        );
        ...
        if (mappedData.length > 0) {
          logger.debug(`Sample mapped data:`, {
            ...
            volumeStats: {
              ...
              avg:
                mappedData.reduce(
                  (sum: number, d: MarketData) => sum + d.volume,
                  0,
                ) / mappedData.length,
            },
          });
        ...
        logger.error(
          `Failed to fetch historical market data for ${symbol}:`,
          error as Error,
        );
        return [];
      }
  ...
  const tradingPort: TradingPort = {
    placeOrder: async (orderParams: OrderParams): Promise<Order> => {
      ...
      try {
        ...
        logger.debug("🏪 Obtention de la configuration du marché...", {
          symbol: orderParams.symbol,
        });
        ...
        const sizeValidation = validateMinimumQuantums(
          orderParams.symbol,
          orderParams.size,
        );
        if (!sizeValidation.isValid && sizeValidation.adjustedSize) {
          logger.warn(
            `🚨 Validation de la taille de l'ordre échouée pour ${orderParams.symbol} : ${sizeValidation.reason}`,
          );
          logger.info(
            `📏 Ajustement de la taille de l'ordre de ${orderParams.size} à ${sizeValidation.adjustedSize}`,
          );
          orderParams.size = sizeValidation.adjustedSize;
        } else if (!sizeValidation.isValid) {
          throw new Error(
            `La taille de l'ordre ${orderParams.size} est inférieure aux exigences minimales pour ${orderParams.symbol} : ${sizeValidation.reason}`,
          );
        }
        ...
        if (
          (orderParams.type === OrderType.LIMIT ||
            orderParams.type === OrderType.STOP_LIMIT) &&
          (priceToUse === undefined || priceToUse === null)
        ) {
          throw new Error(
            "Le prix doit être renseigné pour les ordres LIMIT ou STOP_LIMIT.",
          );
        }
        ...
        if (
          ...
          const latestMarketData = await marketDataPort.getLatestMarketData(
            orderParams.symbol,
          );
        ...
        if (!priceToUse || priceToUse <= 0) {
          throw new Error(
            `Prix invalide calculé pour l'ordre ${orderParams.symbol}: ${priceToUse}`,
          );
        }
        ...
        const finalTimeInForce =
          orderParams.type === OrderType.MARKET
            ? undefined
            : mapTimeInForce(orderParams.timeInForce);

        logger.info(
          "🚀 TRAÇAGE ORDRE - Appel de compositeClient.placeOrder() avec :",
          {
            symbol: orderParams.symbol,
            type: finalType,
            side: finalSide,
        ...
        logger.info(
          "🎯 TRAÇAGE ORDRE - Valeurs EXACTES passées à compositeClient.placeOrder() :",
          {
            arg1_subaccountClient: "[SubaccountClient]",
            arg2_symbol: orderParams.symbol,
            arg3_finalType: finalType,
            arg3_finalType_enum_value:
              Object.keys(DydxOrderType)[
                Object.values(DydxOrderType).indexOf(finalType)
              ],
            arg4_finalSide: finalSide,
            arg4_finalSide_enum_value:
              Object.keys(DydxOrderSide)[
                Object.values(DydxOrderSide).indexOf(finalSide)
              ],
            ...
            arg8_finalTimeInForce_enum_value: finalTimeInForce
              ? Object.keys(OrderTimeInForce)[
                  Object.values(OrderTimeInForce).indexOf(finalTimeInForce)
                ]
              : "undefined",
          },
        );
        ...
        logger.info(
          "🚀 TRAÇAGE ORDRE - APPEL IMMINENT de compositeClient.placeOrder()...",
        );
        ...
        logger.debug("🔍 Récupération de l'ordre depuis l'indexeur...", {
          clientId,
        });
        const client = await getClient();
        ...
        const ordersResponse =
          await client.indexerClient.account.getSubaccountOrders(
            wallet.address || "",
        ...
        if (ordersResponse.orders && ordersResponse.orders.length > 0) {
          logger.debug("🔍 Recherche de l'ordre par clientId...", {
            clientId,
            availableClientIds: ordersResponse.orders.map(
              (o: ExternalOrderData) => o.clientId,
            ),
          });

          const foundOrder = ordersResponse.orders.find(
            (o: ExternalOrderData) => o.clientId === clientId,
          );
          if (foundOrder) {
        ...
        logger.debug(
          "🔧 Création d'un ordre synthétique (ordre non trouvé dans l'indexeur)",
        );
        ...
        if (
          ...
          logger.info(
            `🎉 Order treated as successfully filled due to duplicate prevention for ${orderParams.symbol}`,
            {
          ...
          logger.info(
            `✅ Synthetic filled order created for ${orderParams.symbol} to prevent strategy interruption`,
          );
          return filledOrder;
        }
    ...
    cancelOrder: async (symbol: string): Promise<boolean> => {
      try {
        ...
        logger.error(
          `Échec de l'annulation de l'ordre ${symbol} :`,
          error as Error,
        );
        return false;
      }
    ...
    getOrder: async (orderId: string): Promise<Order | null> => {
      try {
        ...
        logger.error(
          `Échec de la récupération de l'ordre ${orderId} :`,
          error as Error,
        );
        return null;
      }
    ...
    getOpenOrders: async (symbol?: string): Promise<Order[]> => {
      try {
        ...
        logger.error(
          "Échec de la récupération des ordres ouverts :",
          error as Error,
        );
        return [];
      }
    ...
    getAccountBalance: async (): Promise<Record<string, number>> => {
      try {
        ...
        logger.error(
          "Échec de la récupération du solde du compte :",
          error as Error,
        );
        return {};
      }
