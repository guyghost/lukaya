...
import { StrategyService, ParameterRanges } from "../../shared/types";
...
export class BacktestUiAdapter {
  ...
  private strategyService: StrategyService; // Service qui gère les stratégies disponibles
  ...
  async optimizeStrategy(
    ...
    return this.backtestController.optimizeStrategy(
      strategy,
      config,
      parameterRanges,
    );
  ...
  generateBacktestReport(result: BacktestResult): string {
    ...
    report.push(
      `- **Période**: ${result.startDate.toLocaleDateString()} - ${result.endDate.toLocaleDateString()}`,
    );
    report.push(
      `- **Capital initial**: ${result.initialBalance.toFixed(2)} USDT`,
    );
    ...
    report.push(
      `- **Trades gagnants**: ${result.metrics.winningTrades} (${(result.metrics.winRate * 100).toFixed(2)}%)`,
    );
    report.push(`- **Trades perdants**: ${result.metrics.losingTrades}`);
    report.push(
      `- **Gain moyen par trade gagnant**: ${result.metrics.averageWin.toFixed(2)} USDT`,
    );
    report.push(
      `- **Perte moyenne par trade perdant**: ${result.metrics.averageLoss.toFixed(2)} USDT`,
    );
    report.push(
      `- **Plus grand gain**: ${result.metrics.largestWin.toFixed(2)} USDT`,
    );
    report.push(
      `- **Plus grande perte**: ${result.metrics.largestLoss.toFixed(2)} USDT`,
    );
    report.push(
      `- **Facteur de profit**: ${result.metrics.profitFactor.toFixed(2)}`,
    );
    report.push(
      `- **Drawdown maximum**: ${result.metrics.maxDrawdown.toFixed(2)} USDT (${result.metrics.maxDrawdownPercentage.toFixed(2)}%)`,
    );
    report.push(
      `- **Ratio de Sharpe**: ${result.metrics.sharpeRatio.toFixed(2)}`,
    );
    report.push(
      `- **Ratio de Sortino**: ${result.metrics.sortinoRatio.toFixed(2)}`,
    );
    report.push(
      `- **Durée moyenne des positions**: ${(result.metrics.averageHoldingTime / (1000 * 60 * 60)).toFixed(2)} heures`,
    );
    report.push("");
    ...
    report.push("");
    report.push(
      "| # | Date d'entrée | Prix d'entrée | Date de sortie | Prix de sortie | Direction | Profit | Profit % |",
    );
    report.push(
      "|---|--------------|--------------|--------------|--------------|-----------|--------|---------|",
    );
    ...
    result.trades.forEach((trade, index) => {
      ...
      report.push(
        `| ${index + 1} | ${entryDate} | ${trade.entryPrice.toFixed(2)} | ${exitDate} | ${exitPrice} | ${trade.direction} | ${trade.profit.toFixed(2)} | ${trade.profitPercentage.toFixed(2)}% |`,
      );
    });
  ...
  exportBacktestResults(result: BacktestResult, filePath: string): void {
    try {
      ...
      throw new Error(
        `Impossible d'exporter les résultats: ${(error as Error).message}`,
      );
    }
  }
}
