...
import { StrategyService } from "../../shared/types";

/**
 * Adaptateur API REST pour les fonctionnalités de backtesting
 */
export const createBacktestApiAdapter = (
  strategyService: StrategyService, // Service qui gère les stratégies disponibles
) => {
  ...
  router.post("/run", async (req, res) => {
    try {
      ...
      const result = await backtestController.runBacktest(
        strategy,
        backtestConfig,
      );
  ...
  router.post("/compare", async (req, res) => {
    try {
      ...
      if (!strategyIds || !Array.isArray(strategyIds) || !config) {
        return res.status(400).json({
          error:
            "Paramètres manquants ou invalides: strategyIds (tableau) et config sont requis",
        });
      }
      ...
      const results = await backtestController.compareStrategies(
        strategies,
        backtestConfig,
      );
  ...
  router.post("/optimize", async (req, res) => {
    try {
      ...
      if (!strategyId || !config || !parameterRanges) {
        return res.status(400).json({
          error:
            "Paramètres manquants: strategyId, config et parameterRanges sont requis",
        });
      }
      ...
      if (typeof parameterRanges !== "object") {
        return res.status(400).json({
          error:
            "parameterRanges doit être un objet avec des tableaux de valeurs",
        });
      }
      ...
      logger.error(
        "Erreur lors de l'optimisation de la stratégie:",
        error as Error,
      );
      return res.status(500).json({
...
function validateBacktestConfig(
  ...
  if (!config.startDate || !config.endDate) {
    throw new Error(
      "Les dates de début et de fin sont requises dans la configuration",
    );
  }
