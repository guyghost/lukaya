...
export const createHarmonicPatternStrategy = (
        ...
        for (let k = j + 1; k < recentPivots.length - 2; k++) {
          ...
          for (let l = k + 1; l < recentPivots.length - 1; l++) {
            ...
            for (let m = l + 1; m < recentPivots.length; m++) {
              ...
              // Pattern Gartley
              if (
                (isNearFibLevel(AB_XA, fibRatios.r618, tolerance) ||
                  isNearFibLevel(AB_XA, fibRatios.r786, tolerance)) &&
              ...
              // Pattern Butterfly
                ...
                (isNearFibLevel(AB_XA, fibRatios.r786, tolerance) ||
                  isNearFibLevel(AB_XA, fibRatios.r886, tolerance)) &&
                isNearFibLevel(BC_AB, fibRatios.r382, tolerance) &&
              ...
              // Pattern Bat
                ...
                isNearFibLevel(AB_XA, fibRatios.r382, tolerance) &&
                isNearFibLevel(BC_AB, fibRatios.r886, tolerance) &&
  ...
  return {
    ...
    getConfig: () => ({
      ...
      name,
      description:
        "Stratégie basée sur la détection des patterns harmoniques de Fibonacci pour anticiper les retournements de marché",
    ...
    processMarketData: async (
      ...
      if (state.priceHistory.length > config.detectionLength * 2) {
        ...
        state.pivots = identifyPivots(
          state.priceHistory,
          Math.floor(config.detectionLength / 5),
        );
        ...
        if (
          state.currentPatterns.length > 0 &&
          state.lastSignalIndex <
            state.priceHistory.length - config.detectionLength / 2
        ) {
          // Prendre le pattern le plus récent et complet
          const pattern = state.currentPatterns
            .filter((p) => p.completion >= config.patternConfirmationPercentage)
            .sort((a, b) => {
              const maxIndexA = Math.max(
                ...Object.values(a.points).map((p) => p.index),
              );
              const maxIndexB = Math.max(
                ...Object.values(b.points).map((p) => p.index),
              );
          ...
          if (pattern) {
            ...
            if (pattern.direction === "bullish" && state.position !== "long") {
              logger.info(
                `Pattern harmonique ${pattern.type} haussier détecté sur ${data.symbol}`,
              );
              state.position = "long";
            ...
            // Signal d'entrée SHORT: pattern bearish
              ...
              logger.info(
                `Pattern harmonique ${pattern.type} baissier détecté sur ${data.symbol}`,
              );
              state.position = "short";
              state.lastEntryPrice = data.price;
              state.lastSignalIndex = state.priceHistory.length - 1;
            ...
            // Signaux de sortie basés sur les objectifs de prix
              ...
              logger.info(
                `Sortie de position longue sur ${data.symbol} - changement de pattern`,
              );
              state.position = "none";
              state.lastSignalIndex = state.priceHistory.length - 1;
              ...
              logger.info(
                `Sortie de position courte sur ${data.symbol} - changement de pattern`,
              );
              state.position = "none";
              state.lastSignalIndex = state.priceHistory.length - 1;
    ...
    generateOrder: (
      ...
      logger.info(
        `🔍 [DEBUG HARMONIC] Génération d'un ordre pour le signal :`,
        {
    ...
    initializeWithHistory: async (
      ...
      if (!historicalData || historicalData.length === 0) {
        logger.info(
          `Aucune donnée historique fournie pour la stratégie Harmonic Pattern sur ${config.symbol}`,
        );
        return;
      }

      logger.info(
        `Initialisation de la stratégie Harmonic Pattern avec ${historicalData.length} points de données historiques pour ${config.symbol}`,
      );
      ...
      if (state.priceHistory.length > config.detectionLength) {
        state.pivots = identifyPivots(
          state.priceHistory,
          Math.floor(config.detectionLength / 10),
        );
      ...
      logger.info(
        `Stratégie Harmonic Pattern initialisée pour ${config.symbol}`,
        {
          symbol: config.symbol,
